...
@property (copy) NSString *title API_AVAILABLE(macos(10.15)) API_AVAILABLE(ios(13.0));

/*
 When set on an item without a custom view, the button produced will have a bordered style.
 Defaults to NO.
 */
@property (getter=isBordered) BOOL bordered API_AVAILABLE(macos(10.15)) API_AVAILABLE(ios(13.0));
...
- (void)validate API_UNAVAILABLE(ios);
...
@property BOOL autovalidates API_UNAVAILABLE(ios);
...
APPKIT_EXTERN NSToolbarItemIdentifier NSToolbarSeparatorItemIdentifier API_UNAVAILABLE(ios);
...
APPKIT_EXTERN NSToolbarItemIdentifier NSToolbarToggleSidebarItemIdentifier API_AVAILABLE(macos(10.11)) API_AVAILABLE(ios(13.0));  // A standard toolbar item identifier for sidebars. It sends -toggleSidebar: to the firstResponder.

...
/*!
 ...
 @discussion
    This is the only way of setting exposureDuration and ISO. This method throws an NSRangeException if either exposureDuration or ISO is set to an unsupported level. This method throws an NSGenericException if called without first obtaining exclusive access to the receiver using lockForConfiguration:. When using AVCapturePhotoOutput to capture photos, note that the autoStillImageStabilizationEnabled property of AVCapturePhotoSettings defaults to YES, which allows photo capture to temporarily override the capture device's ISO and exposureDuration values if the scene is dark enough to warrant still image stabilization. To ensure that the receiver's ISO and exposureDuration values are honored while in AVCaptureExposureModeCustom or AVCaptureExposureModeLocked, you must set your AVCapturePhotoSettings autoStillImageStabilizationEnabled property to NO. The same rule applies if you use the deprecated AVCaptureStillImageOutput interface — you must set its automaticallyEnablesStillImageStabilizationWhenAvailable property to NO to preserve your custom or locked exposure settings.
 */

...
*/
+ (id)authorization API_AVAILABLE(macos(10.10)) API_UNAVAILABLE(ios, tvos, watchos, uikitformac);

/*!
	@method authorizationRef
	@abstract Returns the AuthorizationRef for this SFAuthorization.
*/
- (AuthorizationRef)authorizationRef API_AVAILABLE(macos(10.10)) API_UNAVAILABLE(ios, tvos, watchos, uikitformac);
...
*/
+ (id)authorizationWithFlags:(AuthorizationFlags)flags rights:(const AuthorizationRights *)rights environment:(const AuthorizationEnvironment *)environment API_AVAILABLE(macos(10.10)) API_UNAVAILABLE(ios, tvos, watchos, uikitformac);
...
/*!
    ...
    @param environment (input/optional) An AuthorizationItemSet containing enviroment state used when making the autorization decision.  See the AuthorizationEnvironment type for details.
*/
- (id)initWithFlags:(AuthorizationFlags)flags rights:(const AuthorizationRights *)rights environment:(const AuthorizationEnvironment *)environment API_AVAILABLE(macos(10.10)) API_UNAVAILABLE(ios, tvos, watchos, uikitformac);

/*!
	@method init
	@abstract Initializes an authorization object initialized with a default environment, flags and rights.
*/
-(id)init API_AVAILABLE(macos(10.10)) API_UNAVAILABLE(ios, tvos, watchos, uikitformac);
...
*/
- (void)invalidateCredentials API_AVAILABLE(macos(10.10)) API_UNAVAILABLE(ios, tvos, watchos, uikitformac);
 ...
 */
- (BOOL)obtainWithRight:(AuthorizationString)rightName flags:(AuthorizationFlags)flags error:(NSError**)error API_AVAILABLE(macos(10.10)) API_UNAVAILABLE(ios, tvos, watchos, uikitformac);
...
/*!
	...
	@param error Resulting error.
*/
- (BOOL)obtainWithRights:(const AuthorizationRights *)rights flags:(AuthorizationFlags)flags environment:(const AuthorizationEnvironment *)environment authorizedRights:(AuthorizationRights **)authorizedRights error:(NSError**)error API_AVAILABLE(macos(10.10)) API_UNAVAILABLE(ios, tvos, watchos, uikitformac);
...
/*!
    ...
    @param authorizedRights (output/optional) A pointer to a newly allocated AuthorizationInfoSet in which the authorized subset of rights are returned (authorizedRights should be deallocated by calling AuthorizationFreeInfoSet() when it is no longer needed).  If NULL the only information returned is the status.  Note that if the kAuthorizationFlagPreAuthorize flag was specified rights that could not be preauthorized are returned in authorizedRights, but their flags contains the kAuthorizationFlagCanNotPreAuthorize bit.
*/
- (OSStatus)permitWithRights:(const AuthorizationRights *)rights flags:(AuthorizationFlags)flags environment:(const AuthorizationEnvironment *)environment authorizedRights:(AuthorizationRights *)authorizedRights DEPRECATED_IN_MAC_OS_X_VERSION_10_5_AND_LATER API_UNAVAILABLE(ios, tvos, watchos, uikitformac);
...
/*!
	...
	@param flags Authorization flags.
*/
- (OSStatus)permitWithRight:(AuthorizationString)rightName flags:(AuthorizationFlags)flags DEPRECATED_IN_MAC_OS_X_VERSION_10_5_AND_LATER API_UNAVAILABLE(ios, tvos, watchos, uikitformac);
